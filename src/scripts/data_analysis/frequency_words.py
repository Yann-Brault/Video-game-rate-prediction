...
def format_name(df):
    ...
    df.to_csv("./dataset/refactored_dataset.csv", index=False)
...
if __name__ == '__main__':
    ...
    # working_df = p.read_csv("./dataset/refactored_dataset.csv")
    working_df = p.read_csv("./dataset/csv/dataset_clean_2.0.csv_repartion_fixed.csv")
    working_df = replace_nan(working_df)
    # show_frequencies(working_df)
    print(working_df['note'].mean())
