...
for i in range(103):
    ...
    try:
        ...
        for j in trange(data_tmp.shape[0]):
            ...
            # data.iloc[acc, 'jeux'] = jeux
            # data.iloc[acc, 'note'] = note
            # data.iloc[acc, 'classe_bon_mauvais'] = cbm
            # data.iloc[acc, 'negative_words'] = neg
            # data.iloc[acc, 'positive_words'] = pos
            # data.iloc[acc, 'avis'] = avis
            # acc+=1
